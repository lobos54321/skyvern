/**
 * Gets the default base URL for parsing relative URLs.
 * Uses window.location.origin in browser, or "http://localhost" in Node/SSR.
 *
 * @param base - Optional base URL to use instead of the default
 * @returns The base URL to use for parsing relative URLs
 */
function getBaseUrl(base?: string): string {
  if (base !== undefined) {
    return base;
  }
...
export function parseUrl(urlString: string, base?: string): URL {
  ...
  try {
    ...
    try {
      ...
      throw new TypeError(
        `Failed to parse URL: ${urlString} with base: ${baseUrl}`,
      );
    }
  }
}
